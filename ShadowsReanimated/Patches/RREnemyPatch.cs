...
public class RREnemyState : State<RREnemy, RREnemyState> {
...
public static class RREnemyPatch {
    ...
    public static void Initialize(RREnemy __instance) {
        var state = RREnemyState.Of(__instance);
    ...
    [HarmonyPostfix]
    [HarmonyPatch(nameof(RREnemy.FlipHorizontally))]
    public static void FlipHorizontally(RREnemy __instance) {
        var state = RREnemyState.Of(__instance);
        state.FlipSpriteIfNeeded();
    }
}
